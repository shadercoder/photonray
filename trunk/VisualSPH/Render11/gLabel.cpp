...
void gLabel::init(CComPtr<ID3D11Device> device, CComPtr<ID3D11DeviceContext> md3dContext, LPCWSTR textFileName)
{
	md3dDevice = device;
	...
	hr = D3DX11CompileFromFile(L"label.hlsl", NULL, NULL, "GS", "gs_4_0", dwShaderFlags, NULL, NULL, &pBlob.p, &err.p, NULL );
	if (FAILED(hr))
	{
		const char* message = (const char*)err->GetBufferPointer();
	...
	hr = D3DX11CompileFromFile(L"label.hlsl", NULL, NULL, "PS", "ps_4_0", dwShaderFlags, NULL, NULL, &pBlob.p, &err.p, NULL );
	if (FAILED(hr))
	{
		const char* message = (const char*)err->GetBufferPointer();
		MessageBoxA(0, message, "Error happens!", MB_OK);
...
void gLabel::draw()
{
	...
	md3dContext->PSSetShaderResources(0, 1, &pTextSRV.p);
