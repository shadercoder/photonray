...
{
	md3dDevice = device;
	DXUT_SetDebugName(md3dDevice, "gMetaballs");
...
{
	SAFE_RELEASE(pFrontS.p);
	SAFE_RELEASE(pFrontSRV.p);
	SAFE_RELEASE(pFrontSView.p);

	SAFE_RELEASE(pBackS.p);
	SAFE_RELEASE(pBackSRV.p);
	SAFE_RELEASE(pBackSView.p);

	SAFE_RELEASE(pNoise.p);
	SAFE_RELEASE(pNoiseSRV.p);
	SAFE_RELEASE(pDepthStencilBuffer.p);
	SAFE_RELEASE(pDepthStencilView.p);
...
{
	HRESULT hr = S_OK;
	//SAFE_RELEASE(p_Volume.p);
	//SAFE_RELEASE(p_VolumeSRV.p);
	//SAFE_RELEASE(p_VolumeUAV.p);
...
{
	HRESULT hr = S_OK;
	//SAFE_RELEASE(p_Particles.p);
	//SAFE_RELEASE(p_ParticlesSRV.p);
	//SAFE_RELEASE(p_ParticlesUAV.p);
...
{
	HRESULT hr = S_OK;
	//SAFE_RELEASE(p_Grid.p);
	//SAFE_RELEASE(p_GridSRV.p);
	//SAFE_RELEASE(p_GridUAV.p);
...
HRESULT gMetaballs::CreateIndexBuffer()
{
	HRESULT hr = S_OK;
/*	
	SAFE_RELEASE(p_Index.p);
	SAFE_RELEASE(p_IndexSRV.p);
	SAFE_RELEASE(p_IndexUAV.p);
